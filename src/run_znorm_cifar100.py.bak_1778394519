"""
znorm+sqrt(n) 主表实验 - CIFAR-100
照搬 run_znorm_scalability.py 骨架, 改动:
  - dataset: prepare_data → prepare_data_cifar100
  - NL: 10 → 100
  - EPB / EPE: 600 → 300/200 (CIFAR-100 类多, 降 epoch)
  - train_experts → train_experts_cifar100 (跳过 < 5 样本的类)
  - N: 10000 (cifar100 test set 也是 10000)
"""
...
def main():
    ...
    NL = 100; FD = 256; LD = 32; EPB = 300; EPE = 200  # CIFAR-100
    torch.manual_seed(SEED); np.random.seed(SEED)

    save_dir = f"saved_models/cifar100_a{ALPHA}_k{NC}_s{SEED}"

    print(f"\n{'='*60}")
    print(f"  znorm CIFAR-100: α={ALPHA}, K={NC}, seed={SEED}")
    print(f"{'='*60}")
    ...
    # 评估
    ed = etf.to(device)
    N = 10000  # CIFAR-100 test set 也是 10000
    all_raw = []; all_labels = []
    ...
    out = {
        'method': 'znorm_sqrt', 'dataset': 'CIFAR-100',
