"""
MoE v15 — 深挖 Per-Client Ensemble (基于 V14/V4)

C4 的成功关键: 保留 per-client 结构, 分别 normalize, 加权融合
比 min 操作好: 各 client 对未见类的错误方向不同 → 加权平均互相抵消

本版深挖维度:
  D1: 权重方案 — uniform, log(n), sqrt(n), n, top-k
  D2: 误差→logit 变换 — -err, exp(-err/τ), 1/(err+ε), rank
  D3: Normalize — z-score, min-max, softmax(τ)
  D4: 自适应 α — union 不确定时增加 expert 权重
  D5: Client 选择 — 只用与 union 一致的 client, 或过滤异常 client
  D6: 多信号融合 — C4 + union margin + expert specificity

训练与 V4 完全相同

运行: python moe_v15_deep_ensemble.py
"""
...
def prepare_data(n_clients=5, alpha=0.05, n_classes=10):
    ...
    for k in range(n_clients):
        ...
        for c,idxs in cm.items():
            dl_kw = dict(num_workers=0,pin_memory=True,persistent_workers=False)
    ...
    for ep in pbar:
        ...
        if should_snap:
            ...
            if save_dir and client_id is not None:
                ...
                torch.save(bb_ckpts[ep_num], os.path.join(sd, f'bb_ep{ep_num}.pt'))
    pbar.close()
    bb.bb_ckpts = bb_ckpts
    return bb
...
# ═══════════════════════════════════════════════════════════
# ★ 策略 D: 深挖 Per-Client Ensemble
# ═══════════════════════════════════════════════════════════

def _build_per_client_errors(data, min_n=0):
    """工具: 返回过滤后的 per-client errors (K, N, nc), inf=无效"""
    K, N, nc = data['errors'].shape
    sc = data['sample_count']
    filtered = data['errors'].clone()
    for k in range(K):
        for c in range(nc):
            if sc.get((k, c), 0) < min_n:
                filtered[k, :, c] = float('inf')
    filtered[filtered == float('inf')] = 1e6
    return filtered  # (K, N, nc)


def _union_norm(data):
    """工具: 返回归一化 union logits"""
    u = data['union_logits']
    um = u.mean(dim=1, keepdim=True); us = u.std(dim=1, keepdim=True) + 1e-8
    return (u - um) / us


def d1_weight_schemes(data, alpha=0.3, min_n=100, weight_type='log'):
    """D1: 不同权重方案
    weight_type: 'uniform', 'log', 'sqrt', 'linear', 'square'
    """
    K, N, nc = data['errors'].shape
    sc = data['sample_count']
    errs = _build_per_client_errors(data, min_n)

    ensemble = torch.zeros(N, nc)
    for k in range(K):
        ek = errs[k]  # (N, nc)
        valid_c = [c for c in range(nc) if sc.get((k, c), 0) >= min_n]
        if not valid_c: continue

        # client logits
        cl = torch.full((N, nc), 0.0)
        for c in valid_c:
            cl[:, c] = -ek[:, c]
        ...
        if cl_mask.any():
            ...
            cs = (diff**2).sum(1, keepdim=True) / cl_mask.sum(1, keepdim=True).clamp(min=1)
            cs = cs.sqrt() + 1e-8
            cl_n = diff / cs
            cl_n[~cl_mask] = 0
            ...
            for c in valid_c:
                n = sc.get((k, c), 0)
                if weight_type == 'uniform': w = 1.0
                elif weight_type == 'log': w = np.log(n + 1)
                elif weight_type == 'sqrt': w = np.sqrt(n)
                elif weight_type == 'linear': w = n / 100.0
                elif weight_type == 'square': w = (n / 100.0) ** 2
                else: w = 1.0
                ensemble[:, c] += cl_n[:, c] * w
    ...
    en = (ensemble - em) / es
    no_signal = (ensemble.abs().sum(1) == 0)
...
def d2_error_transform(data, alpha=0.3, min_n=100, transform='neg', tau=1.0):
    """D2: 误差→logit 变换
    transform: 'neg' (-err), 'exp' (exp(-err/τ)), 'inv' (1/(err+ε)), 'rank'
    """
    K, N, nc = data['errors'].shape
    sc = data['sample_count']
    errs = _build_per_client_errors(data, min_n)
    ...
    for k in range(K):
        ...
        for c in valid_c:
            ...
            if transform == 'neg':
                ...
                cl[:, c] = torch.exp(-e / tau)
            elif transform == 'inv':
                cl[:, c] = 1.0 / (e + 1e-6)
            elif transform == 'rank':
                # rank within each sample across valid classes
                cl[:, c] = -e  # placeholder, rank below
            else:
                cl[:, c] = -e

        if transform == 'rank':
            # per-sample rank across valid classes
            for n_idx in range(N):
                vals = [(cl[n_idx, c].item(), c) for c in valid_c]
                vals.sort(reverse=True)
                for rank, (_, c) in enumerate(vals):
                    cl[n_idx, c] = float(len(vals) - rank)  # higher = better
...
def d3_softmax_ensemble(data, alpha=0.3, min_n=100, tau=0.1):
    """D3: Softmax normalize (而非 z-score)
    per-client: softmax(-error/τ) → 概率 → 加权求和
    """
    K, N, nc = data['errors'].shape
    sc = data['sample_count']
    errs = _build_per_client_errors(data, min_n)
    ...
    ensemble = torch.zeros(N, nc)
    total_weight = torch.zeros(N, nc)
    for k in range(K):
        valid_c = [c for c in range(nc) if sc.get((k, c), 0) >= min_n]
        if not valid_c: continue

        ek = errs[k]  # (N, nc)
        # softmax over valid classes
        logits_k = torch.full((N, nc), -1e6)
        for c in valid_c:
            logits_k[:, c] = -ek[:, c] / tau

        probs_k = F.softmax(logits_k, dim=1)  # (N, nc)
        w = np.log(sc.get((k, valid_c[0]), 0) + 1) if valid_c else 1.0
        ensemble += probs_k * w
        for c in valid_c:
            total_weight[:, c] += w
    ...
    ensemble = ensemble / total_weight  # 加权平均概率
...
def d4_adaptive_alpha(data, base_alpha=0.5, min_n=100):
    """D4: 自适应 α — union margin 低时, 增加 expert 权重
    α(sample) = base_alpha * (1 + (1 - normalized_union_margin))
    union 越不确定 → α 越大 → 越信 expert ensemble
    """
    K, N, nc = data['errors'].shape
    sc = data['sample_count']
    errs = _build_per_client_errors(data, min_n)

    # 构建 ensemble logits (同 C4)
    ensemble = torch.zeros(N, nc)
    for k in range(K):
        valid_c = [c for c in range(nc) if sc.get((k, c), 0) >= min_n]
        if not valid_c: continue
    ...
    # per-sample adaptive alpha
    um = data['union_margin']
    um_norm = (um - um.min()) / (um.max() - um.min() + 1e-8)  # [0, 1]
    per_sample_alpha = base_alpha * (2.0 - um_norm)  # union 不确定 → alpha 翻倍
    per_sample_alpha = torch.tensor(per_sample_alpha).float().unsqueeze(1)

    combined = un + per_sample_alpha * en
    return combined.argmax(1).numpy()


def d5_client_agreement_filter(data, alpha=0.3, min_n=100, agree_thr=2):
    """D5: 只用与主流预测一致的 client
    先各 client 独立预测 → 找多数投票结果 → 只用与多数一致的 client 做 ensemble
    """
    K, N, nc = data['errors'].shape
    sc = data['sample_count']
    errs = _build_per_client_errors(data, min_n)
    ...
    # 各 client 独立预测
    client_preds = torch.full((K, N), -1, dtype=torch.long)
    for k in range(K):
        valid_c = [c for c in range(nc) if sc.get((k, c), 0) >= min_n]
        if not valid_c: continue
        ek = errs[k]; ek_v = torch.full((N, nc), 1e6)
        for c in valid_c: ek_v[:, c] = ek[:, c]
        client_preds[k] = ek_v.argmin(1)

    # 多数投票
    votes = torch.zeros(N, nc)
    for k in range(K):
        for n in range(N):
            p = client_preds[k, n].item()
            if p >= 0: votes[n, p] += 1
    majority = votes.argmax(1)  # (N,)

    # 只用与 majority 一致的 client 做 ensemble
    ensemble = torch.zeros(N, nc)
    for k in range(K):
        valid_c = [c for c in range(nc) if sc.get((k, c), 0) >= min_n]
        if not valid_c: continue
        ...
        # 只在该 client 与 majority 一致时贡献
        agree_mask = (client_preds[k] == majority).float().unsqueeze(1)
        w = np.log(sc.get((k, valid_c[0]), 0) + 1) if valid_c else 1.0
        ensemble += cl_n * w * agree_mask
...
def d6_union_expert_product(data, min_n=100, temp_u=1.0, temp_e=1.0):
    """D6: 概率乘积 — union softmax × expert softmax → 联合概率
    不做加性融合, 做乘性融合 (等价于 logit 相加)
    """
    K, N, nc = data['errors'].shape
    sc = data['sample_count']
    errs = _build_per_client_errors(data, min_n)

    # Expert ensemble logits (同 C4 的 per-client 方式)
    ensemble = torch.zeros(N, nc)
    for k in range(K):
        valid_c = [c for c in range(nc) if sc.get((k, c), 0) >= min_n]
        if not valid_c: continue
    ...
    # 概率乘积 = logit 相加
    u_prob = F.softmax(data['union_logits'] / temp_u, dim=1)
    e_prob = F.softmax(ensemble / temp_e, dim=1)
    joint = u_prob * e_prob  # 乘性融合
    return joint.argmax(1).numpy()
...
def plot_results(results, save_path):
    ...
    for n in names:
        ...
        elif n.startswith('D'): colors.append('#9C27B0')
        else: colors.append('#795548')
    bars = ax.barh(range(len(names)), accs, color=colors, alpha=0.85)
    ax.set_yticks(range(len(names)))
    ax.set_yticklabels(names, fontsize=7)
    ax.set_xlabel('Accuracy (%)')
    ax.set_title('v15: 深挖 Per-Client Ensemble', fontsize=12, fontweight='bold')
    ax.invert_yaxis()
...
def main(ALPHA=0.05):
    print("\n" + "=" * 80)
    print("MoE v15 — 深挖 Per-Client Ensemble")
    print("=" * 80)
    ...
    # ══════════════════════════════════════════════════
    # ★ D: 深挖 Per-Client Ensemble
    # ══════════════════════════════════════════════════
    print(f"\n  --- ★ D: 深挖 Per-Client Ensemble ---")

    # D1: 权重方案
    print(f"\n  D1: 权重方案")
    for wt in ['uniform', 'log', 'sqrt', 'linear', 'square']:
        ...
        print(f"  D1 {wt:8s}: best={best_a:.2%}")

    # D2: 误差→logit 变换
    print(f"\n  D2: 误差→logit 变换")
    for tfm in ['neg', 'exp', 'inv', 'rank']:
        for alpha in [0.2, 0.3, 0.5, 1.0]:
            tau = 0.001 if tfm == 'exp' else 1.0  # exp 需要小 tau
            preds = d2_error_transform(data, alpha=alpha, min_n=100, transform=tfm, tau=tau)
            tag = f'D2 {tfm} α={alpha}'
            R[tag] = (preds == labels).mean()
        best_a = max([R[f'D2 {tfm} α={a}'] for a in [0.2, 0.3, 0.5, 1.0]])
        print(f"  D2 {tfm:5s}: best={best_a:.2%}")

    # D2b: exp 的 tau 扫描
    print(f"  D2b: exp tau 扫描")
    for tau in [0.0001, 0.001, 0.01, 0.1, 1.0]:
        preds = d2_error_transform(data, alpha=0.3, min_n=100, transform='exp', tau=tau)
        tag = f'D2b exp τ={tau}'
        R[tag] = (preds == labels).mean()
        print(f"    τ={tau}: {R[tag]:.2%}")
    ...
    for tau in [0.001, 0.01, 0.1, 1.0]:
        for alpha in [0.2, 0.3, 0.5, 1.0]:
            ...
            R[tag] = (preds == labels).mean()
        best_a = max([R[f'D3 softmax τ={tau} α={a}'] for a in [0.2, 0.3, 0.5, 1.0]])
        print(f"  D3 τ={tau}: best={best_a:.2%}")

    # D4: 自适应 α
    print(f"\n  D4: 自适应 α")
    for base in [0.1, 0.2, 0.3, 0.5, 1.0]:
        for min_n in [50, 100, 200]:
            preds = d4_adaptive_alpha(data, base_alpha=base, min_n=min_n)
            tag = f'D4 adaptive base={base} n≥{min_n}'
            R[tag] = (preds == labels).mean()
        best_a = max([R[f'D4 adaptive base={base} n≥{mn}'] for mn in [50, 100, 200]])
        print(f"  D4 base={base}: best={best_a:.2%}")

    # D5: Client agreement filter
    print(f"\n  D5: Client agreement filter")
    for agree in [2, 3]:
        for alpha in [0.3, 0.5, 1.0]:
            preds = d5_client_agreement_filter(data, alpha=alpha, min_n=100, agree_thr=agree)
            tag = f'D5 agree≥{agree} α={alpha}'
            R[tag] = (preds == labels).mean()
            print(f"  {tag}: {R[tag]:.2%}")

    # D6: 概率乘积
    print(f"\n  D6: 概率乘积")
    for tu in [0.5, 1.0, 2.0]:
        for te in [0.1, 0.5, 1.0, 2.0]:
            preds = d6_union_expert_product(data, min_n=100, temp_u=tu, temp_e=te)
            tag = f'D6 product tu={tu} te={te}'
            R[tag] = (preds == labels).mean()
        best_a = max([R[f'D6 product tu={tu} te={t}'] for t in [0.1, 0.5, 1.0, 2.0]])
        print(f"  D6 tu={tu}: best={best_a:.2%}")

    # D7: min_n 对 C4 最佳配置的影响
    print(f"\n  D7: min_n 扫描 (C4 框架, α=0.3)")
    for min_n in [0, 10, 20, 50, 100, 200, 500, 1000]:
        preds = cross_client_per_client_logits(data, alpha=0.3, min_n=min_n)
        tag = f'D7 C4 α=0.3 n≥{min_n}'
        R[tag] = (preds == labels).mean()
        print(f"  n≥{min_n:4d}: {R[tag]:.2%}")
    # ══════════════════════════════════════════════════
    print(f"\n{'='*70}")
    print(f"★ 最终结果 (Top 40)")
    print(f"{'='*70}")
    ...
    for i, (name, acc) in enumerate(sorted_r[:40]):
        diff_u = acc - baseline_u
        cat = ""
        if name.startswith('A'): cat = "[Agg]  "
        elif name.startswith('B'): cat = "[Route]"
        elif name.startswith('C'): cat = "[Cross]"
        elif name.startswith('D'): cat = "[Deep] "
        marker = " ★" if acc > max(baseline_u, baseline_e) else ""
        print(f"  {i+1:2d}. {cat:8s} {name:<45s} | {acc:>8.2%} "
              f"(vs U {'+' if diff_u>=0 else ''}{diff_u*100:.2f}pp){marker}")

    # 按类别最佳
    print(f"\n  --- 各策略类最佳 ---")
    for prefix, label in [('A', 'Expert聚合'), ('B', 'Union/Expert路由'),
                          ('C', '跨Client交叉'), ('D', '深挖Ensemble')]:
    ...
    plot_results(dict(sorted_r), 'outputs/v15_deep_ensemble.png')
    print(f"\n  训练: {tt:.1f}s\n完成!")


if __name__ == "__main__":
    for alpha in [0.05,0.1,0.3,0.5,1]:
        main(alpha)