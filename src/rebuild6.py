"""
MoE One-Shot FL v4 — 智能 Expert 聚合 + Union/Expert 路由

核心发现: Expert 不需要抗过拟合, 需要更智能的聚合和路由

策略 A: 改进 Expert 聚合 (不改训练, 改 min_k 推理逻辑)
  A1: 质量过滤 — 只用训练样本量 >= thr 的 expert
  A2: 加权平均 — 按训练样本量加权而非 min
  A3: 质量 min — 在质量合格的 expert 中取 min
  A4: Top-Quality — 只用每类训练量最大的 expert
  A5: 加权 min — min 取自质量加权后的误差

策略 B: Union/Expert 路由 (sample-level 二选一)
  B1: Expert margin 路由 — expert top1-top2 误差差大时信 expert
  B2: Union confidence 路由 — union cosine margin 小时信 expert
  B3: 双重信号 — expert margin 大 AND union margin 小 → expert
  B4: 类别路由 — 某些类信 expert, 其他信 union (用验证集学)
  B5: 误差绝对值路由 — expert 预测类误差 < 阈值 → expert
  B6: Ensemble — 不做二选一, 而是用 expert error 修正 union logits

运行: python moe_v4_routing.py
"""
...
def plot_results(results, save_path):
    ...
    ax.set_title('v4: Expert 聚合改进 + Union/Expert 路由', fontsize=12, fontweight='bold')
    ax.invert_yaxis()
...
def main():
    print("\n" + "=" * 80)
    print("MoE v4 — Expert 聚合改进 + Union/Expert 路由")
    print("=" * 80)
    ...
    plot_results(dict(sorted_r), 'outputs/v4_routing_results.png')
    print(f"\n  训练: {tt:.1f}s\n完成!")
