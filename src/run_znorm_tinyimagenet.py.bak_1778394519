"""
znorm+sqrt(n) 主表实验 - Tiny-ImageNet
照搬 run_znorm_scalability.py 骨架, 改动:
  - dataset: prepare_data → prepare_data_tinyimagenet
  - NL: 10 → 200
  - Backbone: ResNet18Backbone → ResNet18Backbone64 (64×64 输入)
  - EPB / EPE: 600 → 300/200
  - train_experts → train_experts_tiny
  - N: 10000 (Tiny test set 大小, 200 类 × 50 张)
"""
...
def main():
    ...
    NL = 200; FD = 256; LD = 32; EPB = 300; EPE = 200  # Tiny
    torch.manual_seed(SEED); np.random.seed(SEED)

    save_dir = f"saved_models/tiny_a{ALPHA}_k{NC}_s{SEED}"

    print(f"\n{'='*60}")
    print(f"  znorm Tiny-ImageNet: α={ALPHA}, K={NC}, seed={SEED}")
    print(f"{'='*60}")
    ...
    # 评估
    ed = etf.to(device)
    # Tiny test: 200 classes × 50 imgs = 10000
    N = 10000
    all_raw = []; all_labels = []
    ...
    for k in range(NC):
        ...
        torch.cuda.empty_cache()
        if (k+1) % 5 == 0: print(f"    eval {k+1}/{NC}")
    ...
    out = {
        'method': 'znorm_sqrt', 'dataset': 'Tiny-ImageNet',
    ...
    path = f"results/znorm_tiny_a{ALPHA}_k{NC}_s{SEED}.json"
